...
try:
    ...
    from joakindex.classificacao import TEXT_EXTENSIONS, SUPPORTED_EXTENSIONS
    ...
    from joakindex.menu_interativo import get_available_ollama_models
except Exception:
    try:
        from .cli import (
            extract_file_author,
        ...
        from .classificacao import TEXT_EXTENSIONS, SUPPORTED_EXTENSIONS
        ...
        from .menu_interativo import get_available_ollama_models
    except Exception:
...
try:
    from joakindex.api_config import (
        handle_get_config,
        handle_get_environments,
        handle_post_config,
        handle_post_config_reset
    )
except ImportError:
...
try:
    from joakindex.api_analysis import (
        handle_get_duplicatas,
        handle_get_duplicatas_comparar,
        handle_post_duplicatas_resolver,
        handle_get_dossies,
        handle_post_dossies_gerar,
        handle_post_dossies_exportar,
...
try:
    from joakindex.api_arquivos import (
        handle_get_info,
...
def create_handler(server_ctx: ConferenciaServer):
    class RequestHandler(SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path == "/api/info":
                handle_get_info(self, server_ctx)
                return
        ...
        def do_POST(self):
            ...
            if path == "/api/duplicatas/resolver":
                handle_post_duplicatas_resolver(self, server_ctx, payload)
                return

            # Geração de Dossiês por Entidades (CPF/CNPJ/Nome)
            if path == "/api/dossies/gerar":
                handle_post_dossies_gerar(self, server_ctx)
                return

            # Exportação de Dossiê Unificado em PDF
            if path == "/api/dossies/exportar":
                handle_post_dossies_exportar(self, server_ctx, payload)
                return
            ...
            if path == "/api/config":
                handle_post_config(self, server_ctx, payload)
                return

            # Restaurar configurações de fábrica via Web
            if path == "/api/config/reset":
                handle_post_config_reset(self, server_ctx)
                return
